...
logger.debug("Testing Sequential Processing")
logger.info("="*50)
...
logger.debug("Testing Parallel Processing (3 workers with independent Tor connections)")
logger.info("="*50)
...
logger.info(f"{'='*50}")
logger.info(f"Sequential: {sequential_time:.1f}s")
logger.debug(f"Parallel (per-worker connections): {parallel_time:.1f}s")
speedup = sequential_time / parallel_time if parallel_time > 0 else 0
logger.info(f"Speedup: {speedup:.2f}x")
logger.info(f"{'='*50}")
logger.debug("\nNote: Each parallel worker used an independent Tor connection,")
logger.info("ensuring better isolation and different exit nodes.")
