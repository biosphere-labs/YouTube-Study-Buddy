...
def test_basic_tracking():
    ...
    logger.success(f"✓ Recorded use of {ip1}: {success}")

    # Try immediate reuse (should fail)
    success = tracker.record_use(ip1, worker_id=2)
    logger.error(f"{'✓' if not success else '✗'} Immediate reuse blocked: {not success}")
    ...
    # Wait for cooldown
    logger.info("  Waiting 4s for cooldown...")
    time.sleep(4)

    # Should be available now
    available = tracker.is_available(ip1)
    logger.success(f"✓ IP available after cooldown: {available}")
...
def test_persistence():
    ...
    logger.success(f"✓ Tracker 1: Recorded {ip}")
    ...
    logger.success(f"✓ Tracker 2: Loaded from disk")
    logger.info(f"  IP available: {available}")
    logger.info(f"  Cooldown remaining: {remaining:.1f}s")

    # Should still be in cooldown
    success = tracker2.record_use(ip, worker_id=2, force=False)
    logger.error(f"{'✓' if not success else '✗'} Cooldown enforced across restart: {not success}")
...
def test_multiple_ips():
    ...
    for i, ip in enumerate(ips):
        ...
        logger.success(f"✓ Recorded {ip} (worker {i})")
    ...
    unavailable = tracker.get_unavailable_ips()
    logger.info(f"  IPs in cooldown now: {', '.join(unavailable) if unavailable else 'none'}")
...
def test_real_world_scenario():
    ...
    logger.debug("Simulating parallel processing with 3 workers:")
    for worker_id, ip in worker_ips.items():
        success = tracker.record_use(ip, worker_id=worker_id)
        logger.error(f"  Worker {worker_id}: {ip} {'✓' if success else '✗'}")
    ...
    logger.info(f"  In cooldown: {stats['in_cooldown']}")
    logger.info(f"  Cooldown period: {stats['cooldown_hours']} hour(s)")
...
def test_cleanup():
    ...
    logger.info(f"Added {len(ips)} IPs")
    logger.info(f"Total tracked: {tracker.get_stats()['total_tracked']}")

    # Wait for expiry
    logger.info("Waiting 5s for entries to expire...")
    time.sleep(5)
...
if __name__ == "__main__":
    logger.info("Exit Node Tracking Test Suite")
    logger.info("=" * 60)
